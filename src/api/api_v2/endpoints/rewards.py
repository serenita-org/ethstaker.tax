...
async def rewards(
    ...
    validator_indexes = set(rewards_request.validator_indexes)
    ...
    first_slot_in_requested_period = BeaconNode.slot_for_datetime(dt=start_datetime)
    activation_slots = await beacon_node.activation_slots_for_validators(validator_indexes=list(validator_indexes), cache=cache)
